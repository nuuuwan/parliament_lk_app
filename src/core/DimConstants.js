...
export const DIM_LIST = [
  ...
  new Dim(  "Voting for 20th Amendment", (mp) => mp.vote20A),
    new Dim("Attandance 9th Parliament (2020 - )", (mp) => mp.attendance9th, true),
    new Dim("Attandance 8th Parliament (2017 - 2020)", (mp) => mp.attendance8th, true),
    ...
    new Dim("Show Random", (mp) => "Random Group " + parseInt(Math.random() * 5 + 1)), true,
];
...
// Derived
export const DIM_IDX = DIM_LIST.reduce(
  function(DIM_IDX, dim) {
...
export const DIM_GROUP_NAME_TO_DIM_LIST = Object.entries(
  ...
  function(DIM_GROUP_NAME_TO_DIM_LIST, [dimGroupName, dimNameList]) {
    DIM_GROUP_NAME_TO_DIM_LIST[dimGroupName] = dimNameList.map(
      function(dimName) {
